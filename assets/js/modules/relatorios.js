...
function renderRelatorios(){
  $('#view-relatorios').innerHTML = `
    <div class="report-grid">
      <div class="report-card" onclick="abrirRelatorioImovel()">
      ...
      <div class="report-card" onclick="abrirRelatorioInquilino()">
      ...
      <div class="report-card" onclick="abrirRelatorioInadimplencia()">
...
function abrirRelatorioImovel(){
  ...
  showModal('Extrato do Imóvel',
    `<form id="repForm">
       <div class="form-grid">
         <div class="field full">
           <label>Imóvel</label>
           <select name="idImovel" onchange="atualizarTiposExtrato(this.value)">${optsImovel}</select>
         </div>
         <div class="field full">
    ...
    `<button class="btn ghost" onclick="closeModal()">Cancelar</button>
     <button class="btn primary" onclick="gerarRelatorioImovel()">Gerar Relatório</button>`);
...
function gerarRelatorioImovel(){
  ...
  const tblReceitas = (rows, vazia) => rows.length ? rows.map(c=>`
    <tr>
      ...
      <td class="money" style="color:var(--emerald);text-align:right">+ ${fmtBRL2(c.Valor_Pago||c.Valor_Cobrado)}</td>
    </tr>`).join('')
    : `<tr><td colspan="5" style="text-align:center;color:var(--txt-mute);padding:14px">${vazia}</td></tr>`;
  ...
  if(tipoFiltro === 'COMPLETO' || tipoFiltro === 'ALUGUEL'){
    secoes += `
      <div style="display:flex;align-items:center;gap:10px;margin:18px 0 10px">
        <div style="width:3px;height:18px;background:var(--amber);border-radius:2px"></div>
        <h4 style="font-family:var(--serif);font-size:15px;font-weight:600;color:var(--amber)">🏠 Aluguéis (${recAlugueis.length})</h4>
      </div>
      <div class="tbl-scroll" style="margin-bottom:4px"><table>
        <thead><tr><th>Data Pgto.</th><th>Tipo</th><th>Inquilino</th><th>Competência</th><th style="text-align:right">Valor</th></tr></thead>
  ...
  if(tipoFiltro === 'COMPLETO' || tipoFiltro === 'ENCARGOS' || ENCARGO_TIPOS.includes(tipoFiltro)){
    ...
    secoes += `
      <div style="display:flex;align-items:center;gap:10px;margin:18px 0 10px">
        <div style="width:3px;height:18px;background:#46b6c4;border-radius:2px"></div>
        <h4 style="font-family:var(--serif);font-size:15px;font-weight:600;color:#46b6c4">💧 Encargos (${rowsEnc.length})</h4>
      </div>
      <div class="tbl-scroll" style="margin-bottom:4px"><table>
        <thead><tr><th>Data Pgto.</th><th>Tipo</th><th>Pago Por</th><th>Competência</th><th style="text-align:right">Valor</th></tr></thead>
  ...
  if(mostrarDespesas){
    const linhasDesp = despesas.length ? despesas.map(m=>`
      <tr>
        ...
        <td class="money" style="color:var(--rose);text-align:right">- ${fmtBRL2(m.Custo||0)}</td>
      </tr>`).join('')
      : `<tr><td colspan="5" style="text-align:center;color:var(--txt-mute);padding:14px">Nenhuma despesa no período</td></tr>`;

    secoes += `
      <div style="display:flex;align-items:center;gap:10px;margin:18px 0 10px">
        <div style="width:3px;height:18px;background:var(--rose);border-radius:2px"></div>
        <h4 style="font-family:var(--serif);font-size:15px;font-weight:600;color:var(--rose)">🔧 Despesas / Manutenções (${despesas.length})</h4>
      </div>
      <div class="tbl-scroll"><table>
        <thead><tr><th>Data</th><th>Manutenção</th><th>Responsável</th><th>Status</th><th style="text-align:right">Custo</th></tr></thead>
  ...
  if(tipoFiltro === 'COMPLETO'){
    summaryCards = `
      <div class="extrato-summary" style="grid-template-columns:repeat(4,1fr)">
        <div class="extrato-box rec"><div class="lbl">Aluguéis</div><div class="val" style="font-size:18px">${fmtBRL2(totalAlugueis)}</div></div>
        <div class="extrato-box" style="background:rgba(70,182,196,.08);border-color:rgba(70,182,196,.2)"><div class="lbl">Encargos <span style="font-size:9px;opacity:.7">(informativo)</span></div><div class="val" style="color:#46b6c4;font-size:18px">${fmtBRL2(totalEncargos)}</div></div>
        <div class="extrato-box desp"><div class="lbl">Despesas</div><div class="val" style="font-size:18px">${fmtBRL2(totalDespesas)}</div></div>
        <div class="extrato-box saldo"><div class="lbl">Saldo <span style="font-size:9px;opacity:.7">(alug. − desp.)</span></div><div class="val" style="font-size:18px">${saldo<0?'- ':''}${fmtBRL2(Math.abs(saldo))}</div></div>
      </div>`;
  } else {
    summaryCards = `
  ...
  const body = `
    <div style="background:var(--bg-soft);border:1px solid var(--line-soft);border-radius:12px;padding:16px;margin-bottom:18px">
      <div style="font-family:var(--serif);font-size:20px;font-weight:600;letter-spacing:-.4px">${esc(im.Nome_Imovel)}</div>
      <div style="font-size:13px;color:var(--txt-dim);margin-top:3px">${esc((im.Endereco_Imovel||'')+(im.Complemento_Imovel?' · '+im.Complemento_Imovel:''))}</div>
      <div style="display:flex;gap:16px;margin-top:6px;flex-wrap:wrap">
        <span style="font-size:13px;color:var(--txt-mute)">📅 Período: ${periodoFmt}</span>
        <span style="font-size:13px;color:var(--amber);font-weight:600">Extrato: ${tipoLabel}</span>
      </div>
    </div>
    ${summaryCards}
    ${secoes}`;
  ...
  showModal('Extrato — '+im.Nome_Imovel, body,
    `<button class="btn ghost" onclick="closeModal()">Fechar</button>
     <button class="btn primary" onclick="gerarExtratoImovelPDF()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>Gerar Relatório</button>`);
...
function abrirRelatorioInquilino(){
  ...
  // Cada contrato é uma locação distinta. Ordena ATIVOS primeiro, depois início
  // mais recente — e rotula com período + status para nunca haver opções iguais.
  const contratosOrd = STATE.contratos.slice().sort((a,b)=>{
  ...
  const opts = contratosOrd.map(c=>{
    const periodo = `${fmtP(c.Data_Inicio_Contrato)||'—'} a ${fmtP(c.Data_Fim_Contrato)||'atual'}`;
    return `<option value="${c.ID_Contrato}" data-ini="${esc(c.Data_Inicio_Contrato||'')}" data-fim="${esc(c.Data_Fim_Contrato||'')}">${esc(c.Nome_Inquilino)} · ${esc(c.Nome_Imovel||'')} · ${periodo} (${optLabel(c.Status_Contrato)})</option>`;
  }).join('');
  const hoje = new Date().toISOString().slice(0,10);
  const umAnoAtras = new Date(); umAnoAtras.setFullYear(umAnoAtras.getFullYear()-1);
  const dtIni = umAnoAtras.toISOString().slice(0,10);
  ...
  showModal('Prestação de Contas do Inquilino',
    `<form id="repForm">
       <div class="form-grid">
         <div class="field full"><label>Contrato (inquilino · imóvel · período)</label><select name="idContrato" onchange="repInqAjustarPeriodo(this)">${opts}</select></div>
         <div class="field"><label>Data Inicial</label><input type="date" name="dtIni" value="${dtIni}"></div>
         <div class="field"><label>Data Final</label><input type="date" name="dtFim" value="${hoje}"></div>
       </div>
       <div style="font-size:12px;color:var(--txt-mute);margin-top:6px">As datas se ajustam ao período do contrato escolhido — você pode alterá-las.</div>
     </form>`,
    `<button class="btn ghost" onclick="closeModal()">Cancelar</button>
     <button class="btn primary" onclick="gerarRelatorioInquilino()">Gerar Relatório</button>`);
  // Ajusta o período ao contrato pré-selecionado
  setTimeout(()=>{ const s=document.querySelector('[name="idContrato"]'); if(s) repInqAjustarPeriodo(s); }, 0);
}

/* Preenche as datas do relatório com o período do contrato selecionado. */
function repInqAjustarPeriodo(sel){
  const opt = sel.options[sel.selectedIndex];
  if(!opt) return;
  const form = sel.form;
  const ini = (opt.getAttribute('data-ini')||'').slice(0,10);
  const fim = (opt.getAttribute('data-fim')||'').slice(0,10);
  const hoje = new Date().toISOString().slice(0,10);
  if(ini && form.elements['dtIni']) form.elements['dtIni'].value = ini;
  if(form.elements['dtFim']) form.elements['dtFim'].value = fim || hoje;  // contrato ativo → até hoje
}
...
function gerarRelatorioInquilino(){
  ...
  const linhasPagas = pagas.length ? pagas.map(c=>`
    <tr>
      ...
      <td class="money" style="color:var(--emerald)">${fmtBRL2(c.Valor_Pago||c.Valor_Cobrado)}</td>
    </tr>`).join('') : `<tr><td colspan="6" style="text-align:center;color:var(--txt-mute);padding:18px">Nenhuma cobrança paga no período</td></tr>`;
  ...
  const linhasPendHtml = linhasPendentes.length ? linhasPendentes.map(c=>{
    ...
    return `<tr>
      ...
      <td style="text-align:center;color:${dias>0?'var(--rose)':'var(--txt-mute)'};font-weight:600">${dias>0?dias+' d':'—'}</td>
      <td class="money" style="color:var(--rose)">${fmtBRL2(c.Valor_Cobrado)}</td>
    </tr>`;
  }).join('') : `<tr><td colspan="6" style="text-align:center;color:var(--txt-mute);padding:18px">Sem pendências no período 🎉</td></tr>`;

  const body = `
    <div style="background:var(--bg-soft);border:1px solid var(--line-soft);border-radius:12px;padding:16px;margin-bottom:18px">
      <div style="font-family:var(--serif);font-size:20px;font-weight:600;letter-spacing:-.4px">${esc(inq.Nome_Inquilino)}</div>
      <div style="font-size:13px;color:var(--txt-dim);margin-top:3px">${im?esc(im.Nome_Imovel):''} ${inq.Telefone_Inquilino?'· '+esc(inq.Telefone_Inquilino):''}</div>
      <div style="font-size:13px;color:var(--txt-mute);margin-top:6px">Período: ${periodoFmt}</div>
    </div>
    <div class="extrato-summary">
    ...
    ${inq.Caucao_Valor ? `
    <div style="background:rgba(224,162,60,.08);border:1px solid rgba(224,162,60,.22);border-radius:12px;padding:14px 18px;margin-bottom:18px;display:flex;align-items:center;gap:18px;flex-wrap:wrap">
      <div>
        <div style="font-size:11px;color:var(--amber);letter-spacing:.5px;text-transform:uppercase;font-weight:600">Caução depositada</div>
        <div style="font-family:var(--serif);font-size:22px;font-weight:700;color:var(--amber);margin-top:2px">${fmtBRL2(inq.Caucao_Valor)}</div>
      </div>
      ${inq.Caucao_Data ? `<div style="font-size:13px;color:var(--txt-dim)">Pago em<br><strong>${new Date(inq.Caucao_Data+'T00:00').toLocaleDateString('pt-BR')}</strong></div>` : ''}
    </div>` : ''}
    <h4 style="font-family:var(--serif);font-size:16px;margin-bottom:10px;color:var(--emerald)">✓ Contas Pagas (${pagas.length})</h4>
    <div class="tbl-scroll" style="margin-bottom:18px"><table>
      <thead><tr><th>Tipo</th><th>Competência</th><th>Vencimento</th><th>Pago em</th><th>Forma</th><th style="text-align:right">Valor</th></tr></thead>
      <tbody>${linhasPagas}</tbody></table></div>
    <h4 style="font-family:var(--serif);font-size:16px;margin-bottom:10px;color:var(--rose)">⚠ Pendências e Atrasos (${linhasPendentes.length})</h4>
    <div class="tbl-scroll"><table>
      <thead><tr><th>Tipo</th><th>Competência</th><th>Vencimento</th><th>Status</th><th style="text-align:center">Atraso</th><th style="text-align:right">Valor</th></tr></thead>
  ...
  showModal('Prestação de Contas — '+inq.Nome_Inquilino, body,
    `<button class="btn ghost" onclick="closeModal()">Fechar</button>
     <button class="btn primary" onclick="gerarPrestacaoContasPDF()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>Gerar Relatório</button>`);
...
function abrirRelatorioInadimplencia(){
  ...
  const blocos = lista.length ? lista.map(g => `
    <div style="border:1px solid var(--line-soft);border-radius:12px;overflow:hidden;margin-bottom:14px">
      <div style="padding:12px 16px;background:var(--bg-soft);display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--line-soft)">
        <div>
          <div class="cell-strong">${esc(g.nome)}</div>
          <div class="cell-sub">${esc(g.imovel)} · ${g.items.length} cobrança(s)</div>
        </div>
        <div class="money" style="color:var(--rose);font-size:18px">${fmtBRL2(g.total)}</div>
      </div>
      <div class="tbl-scroll"><table>
        <thead><tr><th>Tipo</th><th>Competência</th><th>Vencimento</th><th style="text-align:center">Atraso</th><th style="text-align:right">Valor</th></tr></thead>
        <tbody>${g.items.map(c=>{
          const dias = diasAtraso(c.Data_Vencimento, c.Status_Cobranca);
          return `<tr>
            <td>${tipoBadge(c.Tipo_Cobranca)}</td>
            <td>${monthLabel(c.Competencia)}</td>
            <td>${new Date(c.Data_Vencimento+'T00:00').toLocaleDateString('pt-BR')}</td>
            <td style="text-align:center;color:var(--rose);font-weight:600">${dias} d</td>
  ...
  const body = `
    <div class="extrato-summary" style="grid-template-columns:1fr 1fr">
  ...
  showModal('Inadimplência Geral', body,
    `<button class="btn ghost" onclick="closeModal()">Fechar</button>
     <button class="btn primary" onclick="gerarInadimplenciaPDF()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/></svg>Gerar Relatório</button>`);
  document.querySelector('#modalBox').style.maxWidth = '900px';
}
