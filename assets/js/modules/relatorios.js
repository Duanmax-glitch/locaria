...
function gerarRelatorioInquilino(){
  ...
  const body = `
    ...
    ${inq.Caucao_Valor ? `
    <div data-style="background:rgba(224,162,60,.08);border:1px solid rgba(224,162,60,.22);border-radius:12px;padding:14px 18px;margin-bottom:18px;display:flex;align-items:center;gap:18px;flex-wrap:wrap">
      <div>
        <div data-style="font-size:11px;color:var(--amber);letter-spacing:.5px;text-transform:uppercase;font-weight:600">Caução depositada</div>
        <div data-style="font-family:var(--serif);font-size:22px;font-weight:700;color:var(--amber);margin-top:2px">${fmtBRL2(inq.Caucao_Valor)}</div>
      </div>
      ${inq.Caucao_Data ? `<div data-style="font-size:13px;color:var(--txt-dim)">Pago em<br><strong>${new Date(inq.Caucao_Data+'T00:00').toLocaleDateString('pt-BR')}</strong></div>` : ''}
    </div>` : ''}
