       ...
       <div class="form-grid">
         <div class="field full">
           ...
           <input type="text" id="repInqBusca" autocomplete="off" placeholder="Digite o nome do inquilino…"
                  oninput="repInqBuscar(this.value)" onfocus="repInqBuscar(this.value)">
           <div class="rep-busca-box" id="repInqResultados" style="display:none"></div>
         </div>
         <div class="field"><label>Data Inicial</label><input type="date" name="dtIni" value="${dtIni}"></div>
         <div class="field"><label>Data Final</label><input type="date" name="dtFim" value="${hoje}"></div>
       </div>
       <div style="font-size:12px;color:var(--txt-mute);margin-top:6px">Busque pelo nome e escolha o contrato — as datas se ajustam ao período dele.</div>
     </form>`,
    `<button class="btn ghost" onclick="closeModal()">Cancelar</button>
...
function repInqBuscar(q){
  ...
  box.innerHTML = lista.length ? lista.slice(0,50).map(c=>{
    ...
    return `<div class="rep-busca-item" onclick="repInqSelecionar('${c.ID_Contrato}')">
