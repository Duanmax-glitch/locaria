...
function verExtratoPagamento(idCobranca){
  ...
  const linha = (label, valor, destaque) =>
    `<div data-style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--line-soft)">
       <span data-style="font-size:13px;color:var(--txt-dim)">${label}</span>
       <span data-style="font-size:13px;font-weight:600;color:${destaque||'var(--txt)'}">${valor}</span>
     </div>`;

  const body = `
    <div data-style="margin-bottom:6px">
      <div data-style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
        <div class="av" data-style="background:${avColor(c.ID_Imovel)};width:40px;height:40px;font-size:15px">${initials(c.Nome_Imovel)}</div>
        <div>
          <div data-style="font-weight:600;font-size:15px">${esc(c.Nome_Imovel)}</div>
          <div data-style="font-size:12px;color:var(--txt-dim)">${esc(c.Responsavel_Pagamento||'—')}</div>
        </div>
        <div data-style="margin-left:auto">${tipoBadge(c.Tipo_Cobranca)}</div>
      </div>
  ...
  showModal('Extrato do Pagamento', body,
    `<button class="btn ghost" data-action="close-modal">Fechar</button>
     <button class="btn ghost" data-action="comprovante-pdf" data-id="${esc(c.ID_Cobranca)}" title="Abrir comprovante — use o botão de compartilhar do browser para enviar" data-style="color:#e0a23c;border-color:#e0a23c">
       <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15" data-style="margin-right:4px"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
       Comprovante PDF
     </button>
     <button class="btn primary" data-action="close-form-open" data-sheet="cobrancas" data-id="${esc(c.ID_Cobranca)}">Editar</button>`);
}
...
function renderHistorico(){
  ...
  const body = rows.length ? rows.map(c=>{
    ...
    const valorCell = temDesconto
      ? `<div class="money" data-style="color:var(--emerald)">${fmtBRL2(valorPago)}</div>
         <div data-style="font-size:11px;color:var(--rose);font-weight:600;margin-top:2px">desconto ${fmtBRL2(desconto)}</div>`
      : `<span class="money" data-style="color:var(--emerald)">${fmtBRL2(valorPago)}</span>`;
    return `
    <tr data-action="ver-extrato" data-id="${esc(c.ID_Cobranca)}" data-style="cursor:pointer" class="tr-clickable">
      <td><div class="row-flex"><div class="av" data-style="background:${avColor(c.ID_Imovel)}">${initials(c.Nome_Imovel)}</div>
      ...
      <td>${valorCell}</td>
      <td data-action="noop"><div class="act-group">
        <button class="act-btn" title="Ver extrato completo" data-action="ver-extrato" data-id="${esc(c.ID_Cobranca)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
        <button class="act-btn" title="Editar pagamento" data-action="form-open" data-sheet="cobrancas" data-id="${esc(c.ID_Cobranca)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="act-btn del" title="Excluir" data-action="delete" data-sheet="cobrancas" data-key="ID_Cobranca" data-id="${esc(c.ID_Cobranca)}" data-name="${esc(c.Nome_Imovel)} · ${esc(c.Tipo_Cobranca)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div></td>
  ...
  const filtroModoHtml = `
    <div class="panel" data-style="margin-bottom:18px">
      <div class="panel-body">
        <div class="dash-filter" data-current-modo="${filtroHist.modo}">
          <div class="field">
            <label>Modo</label>
            <select data-on-change="hist-field" data-field="modo" data-render="1">
          ...
          <div class="field">
            ...
            <input type="date" value="${esc(filtroHist.inicio)}" data-on-change="hist-field" data-field="inicio">
          </div>
          <div class="field">
            <label>Data Final</label>
            <input type="date" value="${esc(filtroHist.fim)}" data-on-change="hist-field" data-field="fim">
          </div>
          <div class="field btn-wrap">
            <button class="btn primary" data-action="render-historico">Aplicar</button>
          </div>` : ''}
        </div>
      </div>
    </div>`;
  ...
  $('#view-historico').innerHTML = `
    ...
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input id="histSearch" placeholder="Buscar por imóvel, inquilino ou quem pagou…" value="${esc(q)}" data-on-input="hist-field" data-field="q" data-render="1"></div>
    </div>
...
function openPagamentoModal(idCobranca){
  ...
  const infoPagAnterior = c.Status_Cobranca === 'PARCIAL' && valorJaPago > 0 ? `
    <div data-style="background:rgba(224,162,60,.08);border:1px solid rgba(224,162,60,.3);border-radius:8px;padding:10px 14px;margin-top:10px;font-size:13px;color:var(--amber)">
      Já pago anteriormente: <strong>${fmtBRL2(valorJaPago)}</strong> · Restante: <strong>${fmtBRL2(valorRestante)}</strong>
    </div>` : '';

  const body = `
    <div data-style="background:var(--bg-soft);border:1px solid var(--line-soft);border-radius:10px;padding:14px 16px;margin-bottom:16px">
      <div data-style="font-size:12px;color:var(--txt-mute);letter-spacing:.4px;text-transform:uppercase">Cobrança</div>
      <div data-style="font-family:var(--serif);font-size:18px;font-weight:600;margin-top:4px">${esc(c.Nome_Imovel)} · ${tipoBadge(c.Tipo_Cobranca)}</div>
      <div data-style="font-size:13px;color:var(--txt-dim);margin-top:4px">${esc(c.Responsavel_Pagamento)} · Vencimento ${new Date(c.Data_Vencimento+'T00:00').toLocaleDateString('pt-BR')}</div>
      <div data-style="font-family:var(--serif);font-size:24px;font-weight:600;color:var(--amber);margin-top:8px">${fmtBRL2(valorTotal)}</div>
      ${infoPagAnterior}
    </div>
    <form id="payForm">
      <div class="form-grid">
        <div class="field">
          <label>Status *</label>
          <select name="Status_Cobranca" id="payStatus" data-on-change="pay-status-change" data-restante="${valorRestante}">
        ...
        <div class="field">
          ...
          <input type="text" inputmode="numeric" data-money name="Valor_Pago" id="payValor"
            value="${moneyMaskValue(valorRestante)}" placeholder="0,00"
            data-on-input="pay-valor-change" data-total="${valorTotal}" data-pago="${valorJaPago}" required>
        </div>
        <div class="field">
  ...
  showModal('Registrar Pagamento', body,
    `<button class="btn ghost" data-action="close-modal">Cancelar</button>
     <button class="btn primary" data-action="confirmar-pagamento" data-id="${esc(idCobranca)}" data-total="${valorTotal}" data-pago="${valorJaPago}">Confirmar Pagamento</button>`);
...
function onPayStatusChange(valorRestante){
  ...
  if(statusEl.value === 'PAGO'){
    valorEl.value = moneyMaskValue(valorRestante);
...
function onPayValorChange(valorTotal, valorJaPago){
  ...
  const valorAgora = parseMoneyBR(valorEl.value) || 0;
...
async function confirmarPagamento(idCobranca, valorTotal, valorJaPago){
  ...
  const valorPagoAgora = parseMoneyBR(form.elements['Valor_Pago'].value) || 0;
  ...
  if(isDivergente){
    ...
    showModal('Confirmar Desconto',
      `<div data-style="text-align:center;padding:10px 0">
        <div data-style="font-size:44px;margin-bottom:12px">⚠️</div>
        <p data-style="font-size:15px;line-height:1.6;color:var(--txt)">
        ...
        <div data-style="background:rgba(224,162,60,.1);border:1px solid rgba(224,162,60,.3);border-radius:10px;padding:14px;margin:16px 0;font-size:14px;color:var(--amber)">
      ...
      `<button class="btn ghost" data-action="close-open-pay" data-id="${esc(idCobranca)}">Voltar e Corrigir</button>
       <button class="btn" data-style="background:var(--amber);color:#000;font-weight:600" data-action="confirmar-desconto">Sim, aplicar desconto e quitar</button>`);
    return;
  }
