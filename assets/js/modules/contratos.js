...
function renderContratos(){
  ...
  const body=rows.length?rows.map(c=>{
    ...
    return `<tr>
      <td><div class="row-flex"><div class="av" data-style="background:${avColor(c.ID_Imovel)}">${initials(c.Nome_Inquilino)}</div>
      ...
      <td><div class="act-group">
        <button class="act-btn" title="Ver contrato" data-action="view-contrato" data-id="${esc(c.ID_Contrato)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
        ${c.ID_Doc_Contrato
          ? `<button class="act-btn" title="Ver contrato (PDF)" data-action="contrato-pdf" data-id="${esc(c.ID_Contrato)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M9 13h6M9 17h6"/></svg></button>`
          : `<button class="act-btn" title="Criar documento (modelo)" data-action="contrato-doc" data-id="${esc(c.ID_Contrato)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M12 18v-6M9 15h6"/></svg></button>`}
        <button class="act-btn" title="Editar" data-action="form-open" data-sheet="contratos" data-id="${esc(c.ID_Contrato)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="act-btn del" title="Excluir" data-action="delete" data-sheet="contratos" data-key="ID_Contrato" data-id="${esc(c.ID_Contrato)}" data-name="${esc(c.Nome_Inquilino||c.ID_Contrato)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div></td>
    </tr>`;
  ...
  $('#view-contratos').innerHTML=`
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input placeholder="Buscar por inquilino ou imóvel…" value="${esc(f.q)}" data-on-input="filter-search" data-sheet="contratos"></div>
      <div class="seg">${['TODOS','ATIVO','ENCERRADO','SUSPENSO'].map(s=>`<button class="${(f.st||'TODOS')===s?'on':''}" data-st="${s}" data-action="filter-status" data-sheet="contratos">${lbls[s]}</button>`).join('')}</div>
    </div>
...
function viewContrato(idContrato){
  ...
  const vistoriasHtml = vistorias.length ? vistorias.map(v=>{
    ...
    return `<div class="dv-row" data-style="cursor:pointer" data-action="goto-vistoria" data-id="${esc(v.ID_Vistoria)}"><span class="dv-l">${esc(v.Tipo_Vistoria)} · ${d}</span><span class="dv-v">${esc(v.Status_Imovel||'')}</span></div>`;
  ...
  showModal('Reajustar Aluguel',
    `<div class="form-grid">
       ...
       <div class="field"><label>Novo valor do aluguel (R$) *</label>
         <input id="reajNovoValor" type="text" inputmode="numeric" placeholder="0,00" data-money data-on-input="reaj-preview"></div>
       ...
       <div class="field full" id="reajPreviewBox" data-style="font-size:13px;color:var(--txt-dim);min-height:18px"></div>
     </div>`,
    `<button class="btn ghost" data-action="close-modal">Cancelar</button>
     <button class="btn primary" data-action="confirmar-reajuste" data-id="${esc(idContrato)}">Aplicar reajuste</button>`);
}
...
function reajPreview(){
  ...
  const novo = parseMoneyBR((document.getElementById('reajNovoValor')||{}).value) || 0;
  if(!(novo > 0) || !(atual > 0)){ box.innerHTML=''; return; }
  const dif = novo - atual;
  const pct = (dif/atual*100);
  const cor = dif >= 0 ? 'var(--emerald)' : 'var(--rose)';
  const sinal = dif >= 0 ? '+' : '';
  box.innerHTML = `Variação: <strong data-style="color:${cor}">${sinal}${fmtBRL2(dif)} (${sinal}${pct.toFixed(2).replace('.',',')}%)</strong>`;
}
...
function renderReajusteDropdown(){
  ...
  if(!lista.length){
    dd.innerHTML = `<div data-style="padding:24px 16px;text-align:center;color:var(--txt-dim);font-size:13px">Nenhum reajuste a comunicar nos próximos 2 meses. 🎉</div>`;
    return;
  }
  const hoje = new Date().toISOString().slice(0,10);
  const fmt = d => d ? new Date(d+'T00:00').toLocaleDateString('pt-BR') : '—';
  dd.innerHTML = `
    <div data-style="padding:11px 13px 7px;font-weight:600;color:var(--txt);font-size:13px">🔔 Reajustes a comunicar</div>
    ${lista.map(c=>{
      const aniv = aniversarioReajuste(c);
      const prazo = prazoComunicarReajuste(c);
      const prazoVenc = prazo && prazo < hoje;
      return `<div data-style="padding:11px 13px;border-top:1px solid rgba(255,255,255,.06)">
        <div data-style="font-weight:600;color:var(--txt);font-size:13px">${esc(c.Nome_Inquilino||'—')}</div>
        <div data-style="font-size:12px;color:var(--txt-dim);margin-bottom:6px">${esc(c.Nome_Imovel||'—')} · ${fmtBRL2(c.Valor_Aluguel_Contratual)}</div>
        <div data-style="font-size:12px;color:var(--txt-dim)">Aniversário do contrato: <strong data-style="color:var(--txt)">${fmt(aniv)}</strong></div>
        <div data-style="font-size:12px;color:${prazoVenc?'var(--rose)':'var(--amber)'}">Comunicar até: <strong>${fmt(prazo)}</strong>${prazoVenc?' · vencido':''}</div>
        <div data-style="display:flex;gap:8px;margin-top:9px">
          <button class="btn primary" data-style="padding:5px 11px;font-size:12px" data-action="reajustar" data-id="${esc(c.ID_Contrato)}">Reajustar</button>
          <button class="btn ghost" data-style="padding:5px 11px;font-size:12px" data-action="ignorar-reajuste" data-id="${esc(c.ID_Contrato)}">Ignorar</button>
        </div>
      </div>`;
    }).join('')}`;
}
