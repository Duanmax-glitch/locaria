...
function renderInquilinos(){
  ...
  const body=rows.length?rows.map(i=>{
    ...
    return `<tr>
      <td><div class="row-flex"><div class="av" data-style="background:${avColor(i.ID_Inquilino)}">${initials(i.Nome_Inquilino)}</div>
        <div><div class="cell-strong">${esc(i.Nome_Inquilino)}</div><div class="cell-sub">${esc(i.CPF_Inquilino||i.ID_Inquilino)}</div></div></div></td>
      <td><a href="https://wa.me/55${(i.Telefone_Inquilino||'').replace(/\D/g,'')}" target="_blank" data-style="color:var(--emerald);text-decoration:none">${esc(i.Telefone_Inquilino||'—')}</a></td>
      <td>${esc(i.Email_Inquilino||'—')}</td>
      <td>${contratoInfo}</td>
      <td><div class="act-group">
        <button class="act-btn" title="Ver cadastro completo" data-action="view-inquilino" data-id="${esc(i.ID_Inquilino)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
        <button class="act-btn" title="Editar" data-action="form-open" data-sheet="inquilinos" data-id="${esc(i.ID_Inquilino)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="act-btn del" title="Excluir" data-action="delete" data-sheet="inquilinos" data-key="ID_Inquilino" data-id="${esc(i.ID_Inquilino)}" data-name="${esc(i.Nome_Inquilino)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div></td>
    </tr>`;
  ...
  $('#view-inquilinos').innerHTML=`
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input id="inqSearch" placeholder="Buscar por nome ou CPF…" value="${esc(f.q)}" data-on-input="filter-search" data-sheet="inquilinos"></div>
    </div>
...
function viewInquilino(idInquilino){
  ...
  const contratosHtml = cts.length ? cts.map(c=>{
    ...
    return `<div class="dv-row" data-style="cursor:pointer" data-action="goto-contrato" data-id="${esc(c.ID_Contrato)}">
  ...
  showModal(esc(i.Nome_Inquilino), `<div class="detail-view">${body}</div>`,
    `<button class="btn ghost" data-action="close-modal">Fechar</button>
     <button class="btn primary" data-action="form-open" data-sheet="inquilinos" data-id="${esc(i.ID_Inquilino)}">Editar</button>`);
}
