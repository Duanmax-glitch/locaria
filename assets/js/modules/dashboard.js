...
function renderDashboard(){
  ...
  */
  ...
  // ── Aviso de reajuste anual pendente (contratos que passaram de 12 meses) ──
  const reajustePend = contratosParaReajuste();
  const reajusteHtml = reajustePend.length ? `
    <div class="panel" style="margin-bottom:18px;border:1px solid rgba(224,162,60,.4)">
      <div class="panel-head" style="padding-bottom:14px">
        <div>
          <h3 style="color:var(--amber)">🔔 Reajuste de aluguel pendente</h3>
          <div class="sub">${reajustePend.length} contrato(s) completaram 12 meses desde o último reajuste</div>
        </div>
      </div>
      <div class="panel-body" style="padding-top:0">
        <div class="tbl-scroll"><table>
          <thead><tr><th>Inquilino</th><th>Imóvel</th><th>Aluguel atual</th><th>Vencido desde</th><th></th></tr></thead>
          <tbody>${reajustePend.map(c=>{
            const dv = proximoReajuste(c) ? new Date(proximoReajuste(c)+'T00:00').toLocaleDateString('pt-BR') : '—';
            return `<tr>
              <td><div class="cell-strong">${esc(c.Nome_Inquilino||'—')}</div></td>
              <td>${esc(c.Nome_Imovel||'—')}</td>
              <td class="money">${fmtBRL2(c.Valor_Aluguel_Contratual)}</td>
              <td>${dv}</td>
              <td style="text-align:right"><button class="btn primary" style="padding:6px 12px" onclick="reajustarAluguelUI('${c.ID_Contrato}')">Reajustar</button></td>
            </tr>`;
          }).join('')}</tbody>
        </table></div>
      </div>
