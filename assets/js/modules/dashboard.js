  ...
  */
  ...
  const kpiCard = (ic,label,val,foot,svg,tag=null)=>`
    <div class="kpi">
      ...
      <div class="kpi-value" data-style="${tag?'color:var(--emerald)':''}">${val}</div>
  ...
  const pendRows = pend.length ? pend.map(c=>{
    ...
    const valorCell = isParcial ?
      `<div class="money">${fmtBRL2(valorTotal)}</div><div data-style="font-size:11px;color:var(--rose);font-weight:600;margin-top:2px">Devido: ${fmtBRL2(restante)}</div>` :
      `<span class="money">${fmtBRL2(valorTotal)}</span>`;
    // item 5: clicar leva para abrir o modal de pagamento dessa cobrança
    return `<tr class="clickable" data-action="dash-open-pay" data-id="${esc(c.ID_Cobranca)}" title="Clique para registrar pagamento">
      ...
      <td data-style="text-align:center;font-weight:600;color:${dias>0?'var(--rose)':'var(--txt-mute)'}">${dias>0?dias+' d':'—'}</td>
    </tr>`;
  ...
  $('#view-dashboard').innerHTML=`
    <div class="panel" data-style="margin-bottom:18px">
      <div class="panel-body">
        <div class="dash-filter">
          <div class="field">
            <label>Modo</label>
            <select id="dashboardModo" data-on-change="dash-field" data-field="modo" data-render="1">
          ...
          <div class="field">
            ...
            <input type="date" id="dashboardInicio" value="${esc(dashFilter.inicio)}" data-on-change="dash-field" data-field="inicio">
          </div>
          <div class="field">
            <label>Data Final</label>
            <input type="date" id="dashboardFim" value="${esc(dashFilter.fim)}" data-on-change="dash-field" data-field="fim">
          </div>
          <div class="field btn-wrap">
            <button class="btn primary" data-action="render-dashboard">Aplicar</button>
          </div>` : ''}
        </div>
      </div>
    </div>

    <div data-style="display:flex;align-items:center;gap:12px;margin-bottom:12px;margin-top:4px">
      <div data-style="width:4px;height:22px;background:var(--amber);border-radius:2px;flex-shrink:0"></div>
      <span data-style="font-family:var(--serif);font-size:16px;font-weight:600;letter-spacing:-.2px;color:var(--txt)">🏠 Aluguéis</span>
    </div>
    <div class="kpi-grid" data-style="margin-bottom:24px">${row1Html}</div>

    <div data-style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
      <div data-style="width:4px;height:22px;background:#46b6c4;border-radius:2px;flex-shrink:0"></div>
      <span data-style="font-family:var(--serif);font-size:16px;font-weight:600;letter-spacing:-.2px;color:var(--txt)">💧 Encargos <span data-style="font-size:13px;font-weight:400;color:var(--txt-dim)">(Água · Luz · IPTU · Outros)</span></span>
    </div>
    <div class="kpi-grid g4" data-style="margin-bottom:22px">${row2Html}</div>

    <div class="panel" data-style="margin-bottom:22px">
      <div class="panel-head" data-style="padding-bottom:14px">
        <div>
          <h3>Cobranças pendentes / em atraso</h3>
          <div class="sub">Clique em uma linha para registrar o pagamento</div>
        </div>
        <button class="btn ghost" data-action="navigate" data-view="cobrancas" data-style="padding:8px 13px">Ver todas</button>
      </div>
      <div class="panel-body" data-style="padding-top:0">
        <div class="tbl-scroll"><table>
          <thead><tr><th>Imóvel / Inquilino</th><th>Tipo</th><th>Valor</th><th>Vencimento</th><th>Status</th><th data-style="text-align:center">Atraso</th></tr></thead>
    ...
    <div data-style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
      <div data-style="width:4px;height:22px;background:var(--violet);border-radius:2px;flex-shrink:0"></div>
      <span data-style="font-family:var(--serif);font-size:16px;font-weight:600;letter-spacing:-.2px;color:var(--txt)">📊 Consumo Água & Luz <span data-style="font-size:13px;font-weight:400;color:var(--txt-dim)">por imóvel · últimos 6 meses</span></span>
    </div>
    <div data-style="display:flex;flex-direction:column;gap:16px;margin-bottom:22px">
      ${imoveisComAgua.length ? `<div class="panel"><div class="panel-head"><div><h3 data-style="color:#46b6c4">💧 Água</h3><div class="sub">R$ por competência · últimos 6 meses</div></div></div><div class="panel-body"><div class="chart-wrap" data-style="height:240px"><canvas id="chAgua"></canvas></div></div></div>` : ''}
      ${imoveisComLuz.length  ? `<div class="panel"><div class="panel-head"><div><h3 data-style="color:var(--amber)">⚡ Luz</h3><div class="sub">R$ por competência · últimos 6 meses</div></div></div><div class="panel-body"><div class="chart-wrap" data-style="height:240px"><canvas id="chLuz"></canvas></div></div></div>` : ''}
    </div>
    ` : ''}
...
function drawDonut(rec,atr,pend){
  ...
  $('#donutLegend').innerHTML=[['Recebido','#3ec58b',rec],['Atrasado','#f06565',atr],['A vencer','#e0a23c',pend]]
    .map(([l,c,v])=>`<div class="legend-item"><span class="legend-sw" data-style="background:${c}"></span>${l}<span class="legend-val">${fmtBRL(v)}</span></div>`).join('');
}
