...
function renderVistorias(){
  ...
  const tipoBadge=t=>{
    ...
    return `<span style="font-size:11px;font-weight:600;color:${cor};border:1px solid ${cor};padding:2px 8px;border-radius:20px">${esc(t||'—')}</span>`;
  };
  const probBadge=s=>{
    const ok = s==='Sem problemas';
    const cor = ok ? 'var(--emerald)' : '#f06565';
    return `<span style="font-size:11px;font-weight:600;color:${cor}">${esc(s||'—')}</span>`;
  };
  const body=rows.length?rows.map(v=>{
    ...
    return `<tr>
      ...
      <td><div class="act-group">
        <button class="act-btn" title="Comprovante (PDF)" onclick="gerarComprovanteVistoria('${v.ID_Vistoria}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M9 13h6M9 17h6"/></svg></button>
        <button class="act-btn" title="Ver vistoria" onclick="viewVistoria('${v.ID_Vistoria}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
        <button class="act-btn" title="Editar" onclick="openForm('vistorias','${v.ID_Vistoria}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="act-btn del" title="Excluir" onclick="askDelete('vistorias','ID_Vistoria','${v.ID_Vistoria}','${escJs(v.Nome_Imovel||v.ID_Vistoria)}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div></td>
    </tr>`;
  ...
  $('#view-vistorias').innerHTML=`
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input placeholder="Buscar por imóvel ou responsável…" value="${esc(f.q)}" oninput="filters.vistorias.q=this.value;renderVistorias()"></div>
      <div class="seg">${['TODOS','Entrada','Saída','Rotina'].map(s=>`<button class="${(f.st||'TODOS')===s?'on':''}" data-st="${s}" onclick="filters.vistorias.st='${s}';renderVistorias()">${lbls[s]}</button>`).join('')}</div>
    </div>
...
function viewVistoria(idVistoria){
  ...
  showModal(`Vistoria · ${esc(v.Nome_Imovel||'')}`, `<div class="detail-view">${body}</div>`,
    `<button class="btn ghost" onclick="closeModal()">Fechar</button>
     <button class="btn ghost" onclick="gerarComprovanteVistoria('${v.ID_Vistoria}')" style="color:#e0a23c;border-color:#e0a23c"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px;margin-right:5px"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M9 13h6M9 17h6"/></svg>Comprovante PDF</button>
     <button class="btn primary" onclick="openForm('vistorias','${v.ID_Vistoria}')">Editar</button>`);
}
