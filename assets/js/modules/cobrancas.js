...
function renderCobrancas(){
  ...
  const body=rows.length?rows.map(c=>{
    ...
    const valorCell = isParcial ?
      `<div class="money">${fmtBRL2(valorTotal)}</div><div data-style="font-size:11px;color:var(--rose);font-weight:600;margin-top:2px">Restante: ${fmtBRL2(restante)}</div>` :
      `<span class="money">${fmtBRL2(valorTotal)}</span>`;
    return `<tr data-action="open-pay" data-id="${esc(c.ID_Cobranca)}" data-style="cursor:pointer" class="tr-clickable">
      ...
      <td>${new Date(c.Data_Vencimento+'T00:00').toLocaleDateString('pt-BR')}${dias>0?` <span data-style="color:var(--rose);font-size:11px">(${dias}d)</span>`:''}</td>
      <td>${valorCell}</td>
      <td>${statusBadge(c.Status_Cobranca)}</td>
      <td data-action="noop"><div class="act-group">
        <button class="act-btn" title="${isParcial?'Registrar pagamento do saldo':'Marcar como pago'}" data-action="open-pay" data-id="${esc(c.ID_Cobranca)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M20 6 9 17l-5-5"/></svg></button>
        <button class="act-btn" title="Editar" data-action="form-open" data-sheet="cobrancas" data-id="${esc(c.ID_Cobranca)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
  ...
  $('#view-cobrancas').innerHTML=`
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input id="cobSearch" placeholder="Buscar por imóvel ou inquilino…" value="${esc(f.q)}" data-on-input="filter-search" data-sheet="cobrancas"></div>
      <div class="seg">${['TODOS','PENDENTE','PARCIAL','ATRASADO','ATRASADAS_ANTIGAS'].map(s=>`<button class="${f.st===s?'on':''}" data-st="${s}" data-action="filter-status" data-sheet="cobrancas">${lbls[s]}</button>`).join('')}</div>
    </div>
...
function cobFormUpdate(isNew){
  ...
  function onTipoChange(){
    ...
    if(tipo === 'ALUGUEL'){
      ...
      if(con) inpValor.value = moneyMaskValue(con.Valor_Aluguel_Contratual);
      else    inpValor.value = '';
  ...
  function atualizarBadge(){
    ...
    if(!badge){
      ...
      setStyle(badge, 'font-size:12px;font-weight:600;padding:5px 12px;border-radius:8px;display:inline-flex;align-items:center;gap:6px;margin-top:4px');
    ...
    badge.innerHTML = txt + ' <span data-style="opacity:.6;font-weight:400">(automático)</span>';
  }
...
function forcarPreencherValorAluguel(){
   ...
   if(contratoAtivo) inputValor.value = moneyMaskValue(contratoAtivo.Valor_Aluguel_Contratual);
