...
function renderManutencoes(){
  ...
  const body = rows.length ? rows.map(m=>`
    <tr>
      ...
      <td><div class="row-flex"><div class="av" data-style="background:${avColor(m.ID_Imovel)}">${initials(imName(m.ID_Imovel))}</div><div class="cell-strong">${esc(imName(m.ID_Imovel))}</div></div></td>
      ...
      <td class="money">${m.Custo?fmtBRL2(m.Custo):'<span data-style="color:var(--txt-mute)">—</span>'}</td>
      <td>${statusManutBadge(m.Status)}</td>
      <td><div class="act-group">
        <button class="act-btn" title="Editar" data-action="form-open" data-sheet="manutencoes" data-id="${esc(m.ID_Manutencao)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="act-btn del" title="Excluir" data-action="delete" data-sheet="manutencoes" data-key="ID_Manutencao" data-id="${esc(m.ID_Manutencao)}" data-name="${esc(m.Titulo)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div></td>
  ...
  $('#view-manutencoes').innerHTML = `
    <div class="kpi-grid" data-style="grid-template-columns:repeat(3,1fr)">
      <div class="kpi">
    ...
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input id="manutSearch" placeholder="Buscar por título, imóvel ou responsável…" value="${esc(f.q)}" data-on-input="manut-search"></div>
      <div class="seg">${['TODOS','ABERTO','EM_ANDAMENTO','CONCLUIDO','CANCELADO'].map(s=>`<button class="${f.st===s?'on':''}" data-st="${s}" data-action="manut-status">${lblsM[s]}</button>`).join('')}</div>
    </div>
