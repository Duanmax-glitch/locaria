...
function renderImoveis(){
  ...
  const body=rows.length?rows.map(i=>`<tr>
      <td><div class="row-flex"><div class="av" data-style="background:${avColor(i.ID_Imovel)};border-radius:10px"><svg viewBox="0 0 24 24" width="16" fill="none" stroke="#fff" stroke-width="2"><path d="M3 21h18M5 21V7l8-4v18M19 21V11l-6-4"/></svg></div>
      ...
      <td><div class="cell-strong" data-style="font-weight:500">${esc(i.Titular_Conta_Agua||'—')}</div><div class="cell-sub">${i.Cod_Concessionaria_Agua?'cód. '+esc(i.Cod_Concessionaria_Agua):''}</div></td>
      <td><div class="cell-strong" data-style="font-weight:500">${esc(i.Titular_Conta_Luz||'—')}</div><div class="cell-sub">${i.Cod_Concessionaria_Luz?'cód. '+esc(i.Cod_Concessionaria_Luz):''}</div></td>
      <td>${esc(inq(i.ID_Imovel))}</td>
      <td>${statusBadge(i.Status_Atual)}</td>
      <td><div class="act-group">
        <button class="act-btn" title="Detalhes" data-action="view-imovel" data-id="${esc(i.ID_Imovel)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
        <button class="act-btn" title="Editar" data-action="form-open" data-sheet="imoveis" data-id="${esc(i.ID_Imovel)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="act-btn del" title="Excluir" data-action="delete" data-sheet="imoveis" data-key="ID_Imovel" data-id="${esc(i.ID_Imovel)}" data-name="${esc(i.Nome_Imovel)}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div></td>
  ...
  $('#view-imoveis').innerHTML=`
    <div class="table-tools">
      <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input id="imovSearch" placeholder="Buscar por imóvel, titular ou endereço…" value="${esc(f.q)}" data-on-input="filter-search" data-sheet="imoveis"></div>
      <div class="seg">${['TODOS','ALUGADO','VAGO','MANUTENCAO'].map(s=>`<button class="${(f.st||'TODOS')===s?'on':''}" data-st="${s}" data-action="filter-status" data-sheet="imoveis">${lblsIm[s]}</button>`).join('')}</div>
    </div>
...
function viewImovel(id){
  ...
  showModal(esc(i.Nome_Imovel), `<div class="detail-view">${body}</div>`,
    `<button class="btn ghost" data-action="close-modal">Fechar</button>
     <button class="btn primary" data-action="form-open" data-sheet="imoveis" data-id="${esc(i.ID_Imovel)}">Editar</button>`);
}
