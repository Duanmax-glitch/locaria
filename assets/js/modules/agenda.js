...
function renderAgenda(){
  ...
  const cellsHtml = cells.map(cell => {
    ...
    const onclick = hasEvents && !cell.otherMonth ? `onclick="agendaOpenDay('${dateStr}')"` : '';
      ...
      <div class="panel-body">
        <div class="agenda-header">
          ...
          <div class="agenda-nav">
            <button onclick="agendaNav(-1)" title="Mês anterior">‹</button>
            <button onclick="agendaGoToday()" style="font-size:12px;padding:7px 14px">Hoje</button>
            <button onclick="agendaNav(1)" title="Próximo mês">›</button>
          </div>
        </div>
...
function agendaOpenDay(dateStr){
  ...
  const cards = cobs.map(c => {
    ...
    return `
    <div style="background:var(--bg-soft);border:1px solid var(--line-soft);border-radius:14px;padding:16px 18px;display:flex;align-items:center;gap:16px;flex-wrap:wrap">
      <!-- Avatar + nome -->
      <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:180px">
        <div style="width:44px;height:44px;border-radius:12px;background:${avColor(c.ID_Imovel)};display:grid;place-items:center;font-weight:700;font-size:14px;color:#fff;flex-shrink:0">${initials(c.Nome_Imovel)}</div>
        <div>
          <div style="font-weight:600;font-size:14px;letter-spacing:-.2px">${esc(c.Nome_Imovel)}</div>
          <div style="font-size:12px;color:var(--txt-dim);margin-top:1px">${esc(c.Responsavel_Pagamento)}</div>
        </div>
      </div>
      <!-- Tipo -->
      <div style="flex-shrink:0">${tipoBadge(c.Tipo_Cobranca)}</div>
      <!-- Valor -->
      <div style="min-width:110px;text-align:right">
        <div style="font-family:var(--serif);font-size:18px;font-weight:600">${fmtBRL2(valorTotal)}</div>
        ${isParcial ? `<div style="font-size:12px;color:var(--rose);font-weight:600;margin-top:2px">Restante: ${fmtBRL2(restante)}</div>` : ''}
        ${isPago && valorPagoC > 0 && Math.abs(valorPagoC - valorTotal) > 0.009 ? `<div style="font-size:11px;color:var(--txt-mute);margin-top:2px">Pago: ${fmtBRL2(valorPagoC)}</div>` : ''}
      </div>
      <!-- Status -->
      <div style="flex-shrink:0">${statusBadge(c.Status_Cobranca)}</div>
      <!-- Ação / info pago -->
      <div style="flex-shrink:0;min-width:130px;text-align:right">
        ${isPago
          ? `<div style="font-size:12px;color:var(--emerald);font-weight:600">✓ Pago em ${c.Data_Pagamento ? new Date(c.Data_Pagamento+'T00:00').toLocaleDateString('pt-BR') : '—'}</div>
             ${c.Quem_Pagou ? `<div style="font-size:11px;color:var(--txt-mute);margin-top:2px">por ${esc(c.Quem_Pagou)}</div>` : ''}
             ${c.Forma_Pagamento ? `<div style="font-size:11px;color:var(--txt-mute)">${esc(c.Forma_Pagamento)}</div>` : ''}`
          : `<button class="btn primary" style="padding:8px 16px;font-size:13px" onclick="closeModal();setTimeout(()=>openPagamentoModal('${c.ID_Cobranca}'),80)">
               <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" style="width:14px;height:14px"><path d="M20 6L9 17l-5-5"/></svg>
               Registrar Pagamento
             </button>`
        }
      </div>
  ...
  showModal(`📅 ${dtFmt}`,
    `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:16px">
      <div style="background:var(--panel);border:1px solid var(--line-soft);border-radius:10px;padding:10px 8px;min-width:0">
        <div style="font-size:10px;color:var(--txt-mute);letter-spacing:.4px;text-transform:uppercase;margin-bottom:4px">Previsto</div>
        <div style="font-family:var(--serif);font-size:clamp(12px,3.8vw,18px);font-weight:600;color:var(--amber);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtBRL2(total)}</div>
        <div style="font-size:11px;color:var(--txt-mute);margin-top:2px">${cobs.length} cobr.</div>
      </div>
      <div style="background:var(--panel);border:1px solid var(--line-soft);border-radius:10px;padding:10px 8px;min-width:0">
        <div style="font-size:10px;color:var(--txt-mute);letter-spacing:.4px;text-transform:uppercase;margin-bottom:4px">Recebido</div>
        <div style="font-family:var(--serif);font-size:clamp(12px,3.8vw,18px);font-weight:600;color:var(--emerald);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtBRL2(recebido)}</div>
        <div style="font-size:11px;color:var(--txt-mute);margin-top:2px">${cobs.filter(c=>c.Status_Cobranca==='PAGO').length} pago${cobs.filter(c=>c.Status_Cobranca==='PAGO').length!==1?'s':''}</div>
      </div>
      <div style="background:var(--panel);border:1px solid var(--line-soft);border-radius:10px;padding:10px 8px;min-width:0">
        <div style="font-size:10px;color:var(--txt-mute);letter-spacing:.4px;text-transform:uppercase;margin-bottom:4px">Pendente</div>
        <div style="font-family:var(--serif);font-size:clamp(12px,3.8vw,18px);font-weight:600;color:var(--rose);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${fmtBRL2(pendente)}</div>
        <div style="font-size:11px;color:var(--txt-mute);margin-top:2px">${cobs.filter(c=>c.Status_Cobranca!=='PAGO').length} em aberto</div>
      </div>
    </div>
