...
async function gerarPrestacaoContasPDF(){
  ...
  // Caução — apresentada como GARANTIA, no mesmo idioma visual dos KPIs (card
  // claro + acento âmbar), e não como um bloco chapado. Deixa explícito que é
  // valor retido, distinto das contas pagas do período.
  if(d.inq.Caucao_Valor){
    const ch=19;
    doc.setFillColor(248,249,250); doc.roundedRect(ml,y,mr-ml,ch,2,2,'F');        // fundo claro, como os KPIs
    doc.setFillColor(...amber);    doc.rect(ml,y+1.5,1.5,ch-3,'F');                // barra de acento âmbar à esquerda
    doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.setTextColor(100,100,100);
    doc.text('CAUÇÃO DEPOSITADA  ·  GARANTIA LOCATÍCIA',ml+5,y+6);
    doc.setFont('helvetica','bold'); doc.setFontSize(12); doc.setTextColor(...amber);
    doc.text(fV(+d.inq.Caucao_Valor),ml+5,y+13);
    doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.setTextColor(150,150,150);
    doc.text('Valor retido como garantia — não compõe o Total Pago do período.',ml+5,y+17);
    if(d.inq.Caucao_Data){
      doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor(110,110,110);
      doc.text('Depositada em '+fD(d.inq.Caucao_Data),mr-3,y+13,{align:'right'});
    }
    y+=ch+4;
  }
