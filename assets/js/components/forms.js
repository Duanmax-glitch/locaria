...
const FORMS={
  ...
  contratos:{key:'ID_Contrato',title:'Contrato',sections:[
    {nome:'Partes',ico:'<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/>',fields:[
      {k:'ID_Imovel',l:'Imóvel (somente vagos)',t:'imovel',vago:1,req:1,full:1},
...
function fieldHtml(f,rec,isNew){
  ...
  if(f.t==='select'){
    ...
    // f.vago (usado no form de contrato): lista só imóveis SEM contrato ativo —
    // impede dois contratos no mesmo imóvel. Na edição, mantém o imóvel atual do
    // contrato (que aparece "ocupado" justamente por este contrato).
    let lista = STATE.imoveis;
    if(f.vago){
      lista = lista.filter(im => !contratoAtivoDoImovel(im.ID_Imovel) || String(im.ID_Imovel)===String(val));
    }
    const vazio = f.vago && !lista.length;
    input=`<select name="${f.k}"><option value="">${vazio?'— Nenhum imóvel vago —':'— Selecione o imóvel —'}</option>${lista.map(im=>{
    ...
    // Ordena por nome (A-Z, pt-BR, ignorando acento/maiúscula) só na exibição —
    // .slice() para não reordenar o STATE global.
    const lista = STATE.inquilinos.slice().sort((a,b)=>
      String(a.Nome_Inquilino||'').localeCompare(String(b.Nome_Inquilino||''),'pt-BR',{sensitivity:'base'}));
    input=`<select name="${f.k}"><option value="">— Selecione o inquilino —</option>${lista.map(inq=>
