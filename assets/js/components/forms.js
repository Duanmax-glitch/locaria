...
function fieldHtml(f,rec,isNew){
  ...
  if(f.t==='select'){
    ...
    let extraAttrs = isNumeric
      ? `type="text" inputmode="decimal" ${isDia?'min="1" max="31" maxlength="2"':''}`
      : `type="${f.t}"`;
    ...
    input=`<input name="${f.k}" value="${esc(val)}" ${extraAttrs}>`;
  }
...
function openForm(sheet,id){
  ...
  if(cfg.sections){
    const tabs=cfg.sections.map((s,i)=>`<button type="button" class="ftab ${i===0?'on':''}" data-tab="${i}" onclick="switchTab(${i})">
  ...
  showModal(`${isNew?'Novo':'Editar'} ${cfg.title}`, bodyHtml,
    `<button class="btn ghost" onclick="closeModal()">Cancelar</button>
     <button class="btn primary" onclick="submitForm('${sheet}','${id||''}')">Salvar</button>`);
...
function showWarning(id, html){
  ...
  if(!el){
    ...
    el.id = id;
    el.style.cssText = 'background:var(--rose-soft);border:1px solid rgba(240,101,101,.35);color:var(--rose);padding:11px 14px;border-radius:10px;font-size:13px;margin-bottom:14px;line-height:1.5';
        ...
        if(valorPag >= valorCob){
          ...
          showModal('Confirmar Desconto',
            `<div style="text-align:center;padding:8px 0 4px">
               ...
               <p style="font-size:15px;line-height:1.6;color:var(--txt);margin-bottom:16px">
                 Você selecionou <strong>Pago Total</strong>, mas o valor pago
                 <strong style="color:var(--emerald)">${fmtBRL2(valorPag)}</strong>
                 é menor que o cobrado
                 <strong style="color:var(--txt)">${fmtBRL2(valorCob)}</strong>.
               </p>
               <div style="background:rgba(224,162,60,.1);border:1px solid rgba(224,162,60,.3);border-radius:10px;padding:14px;font-size:14px;color:var(--amber)">
            ...
            `<button class="btn ghost" onclick="closeModal();window._pendingSubmitRec=null">Voltar e Corrigir</button>
             <button class="btn" style="background:var(--amber);color:#000;font-weight:600" onclick="escolherTipoPagamento('PAGO')">Sim, aplicar desconto</button>`
          );
          return;
        } else {
          ...
          showModal('Como registrar este pagamento?',
            `<p style="color:var(--txt-dim);line-height:1.7;margin-bottom:16px">
               O valor pago <strong style="color:var(--emerald)">${fmtBRL2(valorPag)}</strong> é menor que o cobrado <strong style="color:var(--txt)">${fmtBRL2(valorCob)}</strong>.
               Como você quer registrar?
             </p>
             <div style="display:flex;flex-direction:column;gap:10px">
               <div onclick="escolherTipoPagamento('PAGO')" style="cursor:pointer;padding:14px 16px;border-radius:12px;border:1px solid var(--emerald);background:var(--emerald-soft);display:flex;align-items:center;gap:12px">
                 <div style="width:36px;height:36px;border-radius:50%;background:var(--emerald);display:grid;place-items:center;flex-shrink:0">
                 ...
                 <div>
                   <div style="font-weight:600;color:var(--emerald)">Desconto / Acordo</div>
                   <div style="font-size:12px;color:var(--txt-dim);margin-top:2px">Restante de ${fmtBRL2(valorCob - valorPag)} descartado. Cobrança encerrada como PAGO.</div>
                 </div>
               </div>
               <div onclick="escolherTipoPagamento('PARCIAL')" style="cursor:pointer;padding:14px 16px;border-radius:12px;border:1px solid var(--blue);background:var(--blue-soft);display:flex;align-items:center;gap:12px">
                 <div style="width:36px;height:36px;border-radius:50%;background:var(--blue);display:grid;place-items:center;flex-shrink:0">
                 ...
                 <div>
                   <div style="font-weight:600;color:var(--blue)">Pagamento Parcial</div>
                   <div style="font-size:12px;color:var(--txt-dim);margin-top:2px">Restante de ${fmtBRL2(valorCob - valorPag)} continua como dívida ativa.</div>
                 </div>
               </div>
             </div>`,
            `<button class="btn ghost" onclick="closeModal();window._pendingSubmitRec=null">Cancelar</button>`
          );
          return;
        }
      } else {
...
function askDelete(sheet,key,id,label){
  showModal('Confirmar exclusão',
    `<p style="color:var(--txt-dim);line-height:1.6">Tem certeza que deseja excluir <strong style="color:var(--txt)">${esc(label)}</strong>?<br>Esta ação não poderá ser desfeita.</p>`,
    `<button class="btn ghost" onclick="closeModal()">Cancelar</button>
     <button class="btn" style="background:var(--rose-soft);color:var(--rose);border-color:rgba(240,101,101,.3)" onclick="doDelete('${escJs(sheet)}','${escJs(key)}','${escJs(id)}')">Excluir</button>`);
}
